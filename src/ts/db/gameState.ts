...
export class GameState extends DbObject {
    ...
    public loadVotes() {
        ...
        for (const voteId of this.votesInProgress) {
            ...
            promises.push(this.votes[voteId].load());
        }

        if (this.lastElection) {
            if (!this.votes[this.lastElection]) {
                this.votes[this.lastElection] = new Vote(this.lastElection);
            }
            promises.push(this.votes[this.lastElection].load());
        }
